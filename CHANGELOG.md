...
items (FF-001 to FF-039) in `docs/changes/2026-09-18-session-3-final/`, one commit each, with a
jsdom check per item in `checks.mjs`, the handback, the teachability plan and the ledger. The
2026-09-17 items it builds on are recorded there too: the §01/§02 merge to one section of 7
minutes, §09 trimmed to 4, §11 gaining the course data rule and a Part 1 brief, and the run sheet
rewritten around a costed clock. Core is 11 sections and 62 planned minutes; the appendix 83; 18
interaction roots, unchanged; the em-dash baseline reproduced at 4 literal plus 3 entity after
every commit; the four §03 preset rankings identical to the decimal before and after.

### Session 3 · what the room meets

§00 prints the session objective above the bridge and says in one sentence who the Coles are.
The bridge's four checks carry letters and render in a scrambled order, so the room answers
before anyone clicks (key C A D B). §03 lists the ten passages by number before ranking. §04's
commit carries the Zoom poll's four options with the same letters. Every sorter's items and
buckets, every quiz option, §06's stages, §07's turns and §11's checklist are numbered or
lettered, and each hint asks one question the room answers aloud. §06 links to Session 2's
seven-step sorter. §11's closing block is the written check against the objective.

### Session 3 · §08, the silent run

The three prompts take the §07 summary as their only input: the follow-up letter with every
sentence traced to a summary line, the task table with "not given" where the summary names no
owner or date, and the next agenda. Each Copy button carries the summary with the prompt. The
summary is rendered in the section from the same constant §07 shows. The four steps of the run
are printed in a panel. Example outputs, one model's answers to prompts 1 and 2, sit behind a
button under each. No vendor application is named anywhere on the page or in its footer.

### Session 3 · coherence and the three non-advisors

§06's chain no longer asserts failures §07 shows did not happen; §07's categorisation output
obeys its own five-heading instruction and becomes the constant §08 runs on; §03's document
sentence accounts for all ten passages; §11 stops assuming a recorded Session 1 baseline; §10
names reverse prompting. One paragraph at §06, and one sentence each at §07 and §11, give the
three learners who do not run client meetings a recurring meeting of their own to work with.
Every section's explanatory paragraphs became a thesis line and bullets: core static text
4,054 to 3,303 words, with every source line, chip, hint, prompt and verbatim panel kept.

### Instructor materials

`instructor-notes/session-3.md` rewritten against the page; a one-page teaching aid
(`session-3-teaching-aid.pdf`, with `.htm` source and `.md` copy) built from the two audits'
measured failure modes; six paste-ready Zoom polls; three Canvas posts drafted from the audits'
verdicts; a verify brief for §09. Register: DW-123 records the V5 timing line red since the
minute cuts.
