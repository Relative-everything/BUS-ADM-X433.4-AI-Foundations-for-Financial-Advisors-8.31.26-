...
Run sheet for Monday 2026-09-21, 6:00 to 9:00 PM Pacific. The page is worked in the room at
core-only depth (decision D1). Nothing in this file addresses a learner.

**The one thing that is different this week: three interactions are run by the room, not
demonstrated. Everything else is narrated.** Session 1 measured 2 of 17 interactions demonstrated
...
- Poll 2 has four options in Zoom; the page's §04 commit has three. The §04 note below says how to
  run both.
...
## The objective, said before 6:19, out loud, once
...
| **8:00** | The seven-step sorter is done. If it has not happened by 8:00 it will not happen. |
| **8:30** | Into the reserve, with the closing check still ahead. |

## The corrections slot, 6:08

Two aloud. Both are things a CFP could act on. Say each with a confidence label.

1. **Output tokens cost five times input, not twice.** Haiku 1 and 5, Sonnet 2 and 10, Opus 5 and
   25, Fable 10 and 50, per million. The cost-per-task conclusion stands; the multiple was wrong.
2. **A missed portability election is not always permanently lost.** For an estate with no filing
   requirement under §6018(a), Rev. Proc. 2022-32 gives a simplified election up to the fifth
   anniversary of death, on a complete Form 706 marked "FILED PURSUANT TO REV. PROC. 2022-32 TO
   ELECT PORTABILITY UNDER § 2010(c)(5)(A)". It supersedes Rev. Proc. 2017-34's two-year window.

**The hook for item 2, which makes it concrete.** Session 2's prompt-critique item asks whether a
client can still elect portability for a spouse who died in 2021. Its answer key names the three
facts the prompt omits: estate size, whether a return was filed, whether the estate was under the
filing threshold. Those three are exactly the Rev. Proc. 2022-32 predicate, and a 2021 death is
inside the five-year window. Tell them to reopen the item.

**Do not read Session 4's watermark appendix aloud to correct the Session 1 watermark claim.** Three
sources disagree: the room was told the watermark traces to an account; Session 4's page says Claude
output is not watermarked at all and marks itself unconfirmed; the note on that same line reports
two search passes finding SynthID applied since August. Until you have read Anthropic's own page
yourself, say only the narrow true thing: **the watermark does not identify an account.** Then stop.

The rest of the Session 1 backlog goes out in writing, not aloud.

## The four polls. Build them in Zoom before the night

A poll configured live already failed once. Read every distribution aloud and either take the branch
or decline it aloud; declining aloud still counts.
...
Ask once. Say nothing for four seconds. Do not reformulate. Session 2 measured 0.57 pauses of three
seconds or more per minute in the AI-mechanics content and exactly **0.00** in the planning content,
which is where all three of these sit.

- **[GATE]** §06, at the Log stage: "Who reads this note, and when?"
- **[GATE]** §09: "Whose consent do you actually need on this call?"
- **[GATE]** §10, before the sorter key opens: "Which of those six is your reading rather than their
  words?"
...
In chat, not aloud. Two open verbal asks in Session 2 returned silence.
...
The page's own closing question stands behind it if the room is talking: the grounded answer in §03
was correctly retrieved, accurately quoted, and wrong. Where in your workflow do you catch it, and
who is holding it if nobody does.
...
the point. The objective is said before the bridge, not after. It is printed on the page above the
bridge, in the panel labelled "Tonight's one thing": read it off the screen.
...
### s6 · §04, 6:38
Fire Poll 2, four options, before the reveal. Then click the room's majority on the page's
three-way commit, which maps (a) and (b) to its first two options and (c) and (d) to its third.
The room usually picks the second.
...
Say it like this: "Four minutes. Nobody talks. Open Claude. On the page, under prompt 1, click
Copy the letter prompt with the summary. Paste it into Claude and send it. Then read what comes
back against the summary on the page. Nothing real, the synthetic summary only." Say what you will
be doing during the silence and then be quiet. Hold the four minutes; you held the Session 1 break
...
This is the course Learning Objective that has received zero minutes. The instrument has been built
and hidden since before Session 2 was taught. Run the sorter, reveal the key, and spend two minutes
on why steps 2 and 4 sit apart from steps 3 and 7.

The seven CFP Board step names carry no source record yet. Do not present the step names as
...
about the delta; the room aggregate is the artefact.
