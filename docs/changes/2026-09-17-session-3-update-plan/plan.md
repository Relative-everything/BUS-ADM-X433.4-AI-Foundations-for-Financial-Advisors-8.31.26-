...
## Appendix. The builder's operating manual

Everything a builder needs to execute §9 without reading `MAINTAINING.md` end to end.

### A.1 The gates, and which are red by design on Session 3

**Three of them do not live in this repository.** `validate_lesson.py`, `validate_dom.js` and
`restyle_sweep.py` ship with the interactive-lesson-builder skill and run **from the skill root**,
not from the repo root, with the repo passed as an argument (`MAINTAINING.md:88-99`):

```
# from the SKILL root, all three must exit 0
...
node    scripts/validate_dom.js    <repo>/session-3/index.html   # needs jsdom
python3 scripts/restyle_sweep.py   <repo>                        # WRITES the fence
```

Everything below runs from the **repository** root:

```
node    scripts/verify-browser.mjs                           # Chromium; DOM, handlers, screenshots
node    scripts/test-case-viewer.mjs                         # the case-facts viewer in Chromium
node    scripts/verify-case.mjs                              # hashes every injected case block
node    scripts/verify-migration.mjs                         # retired facts, arithmetic, timing
node    scripts/verify-sources.mjs                           # the source hash guard
node    scripts/verify-editorial.mjs                         # EDITORIAL.md hard rules, exit 1
node    scripts/verify-style.mjs                             # the managed style fence
node    scripts/test-editorial-regions.mjs                   # 9 tests, the T7 ratchet
node    scripts/build-appendix.mjs   --check                 # minute arithmetic agrees
node    scripts/inject-sources.mjs   --check                 # footer matches SOURCES.md
node    scripts/inject-case.mjs      --check                 # case blocks not stale
node    scripts/build-unsourced.mjs  --check                 # docs/unsourced-claims.md current
node    scripts/build-bibliography.mjs --check               # the three generated files
node    scripts/case-inventory.mjs   --report-check          # the drift surface
```

`restyle_sweep --check` is the skill's, run from the skill root against `<repo>`.

**Red by design on Session 3 today. Do not "fix" these; a green line here is the anomaly.**

| Line | Why it is tolerated |
|---|---|
| `validate_lesson` V2, 5 FAILs, one per footer citation hyperlink | DW-029, upstream: V2 matches any `href` |
| `validate_lesson` V6, "18 distinct interactions (band 13-15)" | DW-094, by instruction. Note V6 **passes** on type-count and adjacency; only the band fails |
| `validate_dom` "#pnum did not flip" | DW-112, upstream: the corpus renamed it `#ovr` |
| `restyle_sweep --check`, two fragments without a fence | documented |
| `verify-sources` one ADVISE on `src-finra2409` | Session 4's, DW-021's remaining half |
| `validate_lesson` V4, three bare chips | the footer legend |

**Inverted on Session 3, and worth knowing before you touch DW-117:** because Session 3 keeps the
Shift+U override, **V8 passes and DOM mode marks 17 of 17 gates**. Remove the override and Session 3
inherits Session 2's two red lines (DW-116). This is the arithmetic behind the recommendation in §6.2.

### A.2 The em-dash ratchet

`scripts/editorial-baseline.json`, current Session 3 entries:

```json
"A8": { "files": { "session-3": { "majority": "literal", "minority_form": "entity", "minority": 15 } } }
"A9": { "files": { "session-3": { "literal": 6, "entity": 3, "total": 9 } } }
```

A9 fell from 86 literal / 12 entity / 98 total to 6 / 3 / 9 across the 2026-09-13 pass, recorded as
sixteen dated lines on DW-122. A8's minority of 15 measures 3, which is DW-053's twelve units of slack.

**The landing procedure after any edit that moves either count**, copied from the Session 2 handback:

1. Run `node scripts/test-editorial-regions.mjs` and read the `session-3` line.
2. If a count **fell**, re-record the new figure in `editorial-baseline.json` and append a dated line
   to DW-122 giving the before and after.
3. If a count **rose**, halt. A rise is not landable without an instruction.
4. Require T7 green and a clean `validate_dom` load before committing.

### A.3 Generated regions: never hand-edit inside a sentinel pair

| Region | Sentinels | Regenerated by |
|---|---|---|
| The appendix contents panel | `APXPANEL:BEGIN` / `:END` | `node scripts/build-appendix.mjs` |
| The minutes-by-section table | `APXBUDGET:BEGIN` / `:END` | same |
| Each appendix section's stub | `APXSTUB:BEGIN` / `:END` | same |
| The footer source list | `SOURCES:BEGIN v1` / `:END` | `node scripts/inject-sources.mjs` |
| The injected case block | the CASE sentinels | `node scripts/inject-case.mjs` |

### A.4 If you change any section's minutes

A7 and A8 both do. Every one of these must be updated or regenerated, or V5 fails:

1. The section's own `<span class="mins">N min</span>` in its eyebrow. **Hand-edited.**
2. The `APXBUDGET` table. **Regenerated**, `node scripts/build-appendix.mjs`.
3. The appendix contents panel's "The 12 sections of the core session run in about 67 minutes"
   sentence and the `Allocated` row. **Regenerated by the same command.**
4. `instructor-notes/session-3.md`. **Hand-edited**, and this is the one the generators cannot see.
5. `docs/audits/profiles/session-3.json` if you want the profiler to agree.

Then `node scripts/build-appendix.mjs --check` must report current, and `validate_lesson` V5 must
report the three numbers agreeing.

### A.5 CASE.md Part O is out of scope

Part O holds the ten chunks Session 3 §03 ranks and the five meeting turns. The page builds an IDF
index over them at load, so **a one-word edit to a chunk moves every score and the margin flag.**

Nothing in §9 touches Part O, and nothing should. If a change seems to require it, halt and name the
ranking consequence rather than making the edit.

**The required before-and-after check** whenever §03 or Part O is in scope, from the 09-13 pass:
...
Run the four presets in Chromium before and after. Any change to these four rows is a halt.

### A.6 The change-folder procedure

1. Create `docs/changes/2026-09-17-session-3-update-plan/` (done; this file is its `plan.md`).
2. Append the instructor's notes verbatim to `notes-verbatim.md` as they arrive.
3. Open `ledger.md` with one row per atomic item: id, note line, the note's first ten words, the
   anchor as section id plus `data-task` plus a quoted string plus a line number at the starting
   HEAD, confidence, class, priority, cross-referenced DW rows, the planned edit, the test, the
   commit, the result, the status. Ids run `JN-001` upward and restart per folder.
4. Write `checks.mjs` with one assertion per item that a DOM check can prove.
5. Work in groups of about five items. **One commit per item.** Push after each group.
6. After every structural commit: regenerate, run the gates, re-record T7 if it fell.
7. Close with `handback.md`: what changed, the syllabus mapping, what to watch for on screen, the
   full verification output, self-review findings, rejected options, and numbered flags.

### A.7 Standing prohibitions

- **No live-model console on Sessions 2 to 4.** `MAINTAINING.md`. A request for one is a halt.
- **No date, deadline, grade or weight on any page.** `MAINTAINING.md:298` Tier A and `:299` Tier B.
  This is why §7.4 forbids fixing F29 on a page.
- **Never write `last_verified` from a build.** DW-024: only a human at a terminal may write it.
- **Egress-blocked hosts**: anthropic.com, artificialanalysis.ai, livebench.ai, openai.com,
  arxiv.org, reglab.stanford.edu, irs.gov. platform.claude.com is reachable. A figure from a blocked
  host is carried at its recorded date or marked, never re-verified from a build.
