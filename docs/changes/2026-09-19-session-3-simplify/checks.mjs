...
{ const m = d.querySelectorAll('#ppMeet button'); if (m[2]) m[2].click(); const c = d.querySelectorAll('#ppAreas input'); [0,1].forEach(i => { c[i].checked = true; c[i].dispatchEvent(new dom.window.Event('change', {bubbles:true})); }); say(/prepare for a recurring meeting that is written up afterwards\./.test(txt('#ppOut')) && !/client fact/.test(txt('#ppOut')), 'SM-029', 'the third meeting type reads as prompt text'); say(!/Advisors were rating|opening the top passage|Vote, defend/.test(html), 'SM-029', 'C3 and C4 no longer answer or stage-direct ahead of the learner'); }
/* SM-ITEMS */
